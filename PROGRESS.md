...
- [x] frontend-design taste pass (2026-09-19, anthropics skill installed to .agents/skills): pitch as XI hero (moved above verdict), programme masthead (clamp + tight tracking + balance), sentence-case section heads (dropped tracked ALL-CAPS eyebrow default), radius hierarchy (cards 14px), mow-stripe bands on pitch (hard-stop flats, the one bold element), removed redundant pitch-caption rule (Chanel), verdict 70ch measure. No new deps.
- [x] resilient fpl-client + true selling price (2026-09-20): lib/fpl-client.ts (BROWSER_HEADERS Chrome131 + Sec-Fetch + retry 403/429/502/503 x3 600/1500ms, 9s abort, FPL_MIRROR fallback), lib/selling-price.ts (transfers walk + history tenths + floor(profit/2)), app/api/fpl/selling-price route, transfer-planner on-demand 1-fetch per sale (not 15) to stay in Vercel 10s.
- [x] planner suggestions v1 (2026-09-20, d7d534b): lib/suggest.ts rankWeak/suggestReplacements/buildSuggestions (same-position affordable + score delta), transfer-planner Suggested moves strip + Apply (FT/-4) + sequential sell-price fetch for weak 4.
- [x] long-term horizon + crowd + profit runway mitigations (2026-09-20, 62c148f): horizonScores 3GW totals + per-GW splits, rankWeak with blanks*3 + inXi penalty + avail gates (0 then <0.75), suggestReplacements primary hDelta - blanks*4 (fallback allows blanks), momentum hot>50k/warm>10k/cold from net transfers, VALUE badge ≤£6m+hot, 1GW spike flag (>70% in one GW), crowdTop11 + collapsible crowd top 11 + hint citing 15m predictor cadence.
- [x] harness agents model fix (2026-09-20): /root/.omo/omo.jsonc — replaced invalid deepseek-v4-flash-free → jev-1.13-free, ling-3.0-flash-free → ling-3.0-flash-fin-free, north-mini-code-free → mimo-v2.5-free per provider error (retry proved north-mini also 404); re-prioritized fallbacks cheapest-first (jev/mimo → big-pickle → nemotron for ultrabrain/oracle/momus).
...
- Subagent models fixed 2026-09-20: deepseek→jev-1.13-free, ling→ling-fin-free, north-mini→mimo-v2.5-free (all 404 per provider); fallback chain re-prioritized cheapest-first — smoke test now passes.
